    ...
    function($ionicTemplateLoader, $ionicBackdrop, $ionicPlatform, $q, $timeout, $rootScope, $document) {
        return {
            ...
            link: function(scope, element, attrs, ngModel) {
                ...
                ngModel.$render = function(){//TODO Add posibility to pass render function
                    if(!ngModel.$viewValue){
                      ...
                      //TODO TUNE VIEW
                       ...
                       ngModel.$viewValue.address_components.map(function(component){
                         var localityTypes = [];
                         var provinceTypes = [];
                         component.types.map(function(type){
                           if(type==='locality')
                            localityTypes.push(type);
                           else if(type==='administrative_area_level_1')
                             provinceTypes.push(type);
                         });
                         if(localityTypes.length>0)
                           cities.push(component);
                         else if(provinceTypes.length>0)
                           provinces.push(component);
                       });

                       var city = cities[0];
                       var province = provinces[0];
                      element.val(city.long_name + ', '+province.long_name);
